...
export function useDisparos(getPlayerX, getPlayerWidth, volumenDisparo = 0.3, pausado = false) {
  ...
  useEffect(() => {
    ...
    if (audioRef.current) audioRef.current.volume = volumenDisparo;
  }, [volumenDisparo]);
  ...
  useEffect(() => {
    const down = (e) => { if (e.code === "Space") { e.preventDefault(); spacePressed.current = true; } };
  ...
  useEffect(() => {
    const loop = (time) => {
      ...
      if (!pausadoRef.current) {
        const disparando = spacePressed.current || touchShootRef.current;
        ...
        if (disparando) {
          ...
          if (now - lastShotRef.current >= COOLDOWN) {
            ...
            setDisparos((prev) => [
              ...prev,
              { id: now, x: playerX + playerWidth / 2 - 18, y: window.innerHeight - 120, explota: false },
            ]);
          }
        }
        ...
        setDisparos((prev) =>
          prev
            .map((d) => ({ ...d, y: d.explota ? d.y : d.y - VELOCIDAD_DISPARO * delta }))
            .filter((d) => {
  ...
  const explotarDisparo = useCallback((id) => {
    setDisparos((prev) =>
      prev.map((d) => d.id === id ? { ...d, explota: true, explotaTime: Date.now() } : d)
    );
  }, []);

  return { disparos, explotarDisparo, touchShootRef };
}
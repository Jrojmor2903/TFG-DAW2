    ...
    <div className="background-general min-h-screen p-4 md:p-8 text-white w-full box-border overflow-x-hidden">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-400 text-sm mt-2">Explora y filtra los desafíos creados por los jugadores</p>
        </div>
        ...
        <div className="bg-black/40 border border-gray-800 p-4 rounded-2xl flex flex-col sm:flex-row gap-4 w-full box-border">
          ...
          <div className="flex-1 flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Buscar por Nivel</label>
            <input
              type="text"
          ...
          <div className="w-full sm:w-64 flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Filtrar por Jugador</label>
            <select
          ...
          {(filtroNombre || filtroCreadorId) && (
            <div className="flex items-end">
              <button
                onClick={() => { setFiltroNombre(""); setFiltorCreadorId(""); }}
        ...
        {nivelesFiltrados.length === 0 ? (
          <div className="text-center py-12 bg-black/20 border border-dashed border-gray-800 rounded-2xl">
            <p className="text-gray-500">No se encontraron niveles con los filtros aplicados.</p>
          </div>
        ) : (
            ...
            {nivelesFiltrados.map((nivel) => (
              ...
              >
                ...
                <div className="p-5 flex flex-col gap-3 flex-1">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Creado por: <span className="text-[var(--tema-visual)] font-semibold">{nivel.creador?.name || `ID: ${nivel.id_creador}`}</span>
                    </p>
                  </div>
                  ...
                  {nivel.enemigos && nivel.enemigos.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {nivel.enemigos.slice(0, 3).map((e, index) => (
                        <span key={index} className="text-[10px] bg-neutral-800 border border-neutral-700 px-2 py-0.5 rounded-full text-gray-300">
                          {e.nombre || `Enemigo #${e.id}`} x{e.pivot?.cantidad || e.cantidad}
                        </span>
                      ))}
                      {nivel.enemigos.length > 3 && (
                        <span className="text-[10px] text-gray-500 pl-1">+{nivel.enemigos.length - 3} más</span>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="p-5 pt-0">
                  <button className="w-full py-2.5 bg-neutral-800 hover:bg-[var(--tema-visual)] hover:text-black font-bold text-sm rounded-xl transition-all">
                    Jugar Nivel
                  </button>
                </div>
              </div>

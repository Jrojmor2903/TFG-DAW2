...
export default function AdminDashboard() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (modalType === "crear") {
        ...
        setData(prev => prev.map(item => item.id === currentEntity.id ? (res.data.data || res.data) : item));
  ...
  // ── Handler específico para niveles ──
                ...
                <tr key={item.id} className="hover:bg-neutral-900/30 transition-colors">
                  {currentCols.map(col => (
                    <td key={col} className="p-4 text-xs md:text-sm max-w-[150px] md:max-w-xs truncate font-mono text-neutral-200">
                      {col === "rol" ? (
                        (() => {
                          if (Array.isArray(item.roles) && item.roles.length > 0) return item.roles.map(r => r.nombre || r.name).join(", ");
                          if (Array.isArray(item.asignaciones) && item.asignaciones.length > 0) return item.asignaciones.map(a => a.rol?.nombre || `Rol #${a.rol_id}`).join(", ");
                          if (item.rol && typeof item.rol === 'object') return item.rol.nombre || item.rol.name || "Asignado";
                          return "Sin Rol";
                        })()

...
const SECTIONS = {
  ...
  niveles: { label: "Niveles", endpoint: "/nivel", columns: ["id", "nombre_nivel", "dificultad" , "fondo_url"] },
  enemigos: { label: "Enemigos", endpoint: "/enemigo", columns: ["id", "nombre", "vida", "daño", "imagen_url"] }
};
...
export default function AdminDashboard() {
  ...
  // Estado para roles dinámicos del backend
  const [rolesBackend, setRolesBackend] = useState([]);
  const [verPapelera, setVerPapelera] = useState(false);
  const [currentEntity, setCurrentEntity] = useState(null);
  const [modalType, setModalType] = useState(null);

  // Carga automática de datos de la sección activa
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (activeTab === "usuarios" && verPapelera) {
          url = "/user/deleted"; 
        }
        ...
        console.warn(`El endpoint para ${activeTab} no está disponible o devolvió error:`, err.message);
        setData([]); 
      } finally {
        setLoading(false);
      }
  ...
  // Carga los roles del backend al montar el componente para el selector
  useEffect(() => {
    async function cargarRoles() {
      try {
        ...
        console.error("No se pudieron cargar los roles dinámicos del backend:", err);
      }
    }
    cargarRoles();
  }, []);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert("Error al eliminar el recurso. Comprueba que el controlador admita DELETE.");
    }
  };
  ...
  const handleForceDeleteUser = async (id) => {
    if (!window.confirm("⚠️ ATENCIÓN: ¿Deseas borrar permanentemente este usuario?")) return;
    try {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (modalType === "crear") {
        ...
        setData(prev => prev.map(item => item.id === currentEntity.id ? updatedData : item));
        alert("Registro actualizado con éxito.");
      }
      setModalType(null);
    } catch (err) {
      console.error("Error detallado en el servidor:", err.response?.data || err);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neutral-950 text-green-400 font-mono flex items-center justify-center tracking-widest animate-pulse">
        SINCRO-DATOS: CARGANDO {verPapelera ? "PAPELERA DE" : "REGISTROS DE"} {activeTab.toUpperCase()}...
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-neutral-800 pb-6 mb-8 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white">
            Panel de Control <span className="text-[var(--tema-visual,rgb(34,197,94))]">Admin</span>
          </h1>
        </div>
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          {activeTab === "usuarios" && (
            <button
              ...
              className={`px-5 py-3 font-bold rounded-xl border transition-all text-sm ${
                verPapelera ? "bg-red-500/20 text-red-400 border-red-500" : "bg-neutral-900 text-neutral-400 border-neutral-800 hover:text-white"
              }`}
            >
          ...
          {!verPapelera && (
            <button 
              onClick={() => { setCurrentEntity({}); setModalType("crear"); }}
        ...
        {Object.entries(SECTIONS).map(([key, config]) => (
          <button
            ...
            className={`snap-origin-left px-5 py-2.5 rounded-t-xl font-semibold transition-all duration-200 whitespace-nowrap ${
              activeTab === key ? "bg-neutral-800 text-[var(--tema-visual,rgb(34,197,94))] border-b-2 border-[var(--tema-visual,rgb(34,197,94))]" : "text-neutral-400 hover:text-white hover:bg-neutral-900"
            }`}
          >
            {config.label}
          </button>
        ...
        <table className="w-full min-w-[700px] md:min-w-full text-left border-collapse table-auto">
          <thead>
            <tr className="bg-neutral-950 border-b border-neutral-800 text-neutral-400 text-xs md:text-sm font-semibold uppercase tracking-wider">
              {currentCols.map(col => <th key={col} className="p-4">{col}</th>)}
          ...
          <tbody className="divide-y divide-neutral-800/60">
            {data.length === 0 ? (
              <tr>
                <td colSpan={currentCols.length + 1} className="p-8 text-center text-neutral-500 font-mono text-xs md:text-sm">
                  {verPapelera ? "La papelera está vacía." : `Módulo [${activeTab.toUpperCase()}] sin registros.`}
                </td>
              </tr>
            ) : (
              data.map((item) => (
                <tr key={item.id} className="hover:bg-neutral-900/30 transition-colors">
                  {currentCols.map(col => (
                    <td key={col} className="p-4 text-xs md:text-sm max-w-[150px] md:max-w-xs truncate font-mono text-neutral-200">
                      {col === "rol" ? (
                        (() => {
                          ...
                          if (Array.isArray(item.asignaciones) && item.asignaciones.length > 0) {
                            return item.asignaciones.map(asig => asig.rol?.nombre || `Rol #${asig.rol_id}`).join(", ");
                          }
                  ...
                  <td className="p-4 flex gap-1.5 justify-center items-center whitespace-nowrap">
                    {activeTab === "usuarios" && verPapelera ? (
                      <>
                        <button onClick={() => handleRestoreUser(item.id)} className="px-3 py-1.5 bg-green-500/10 text-green-400 border border-green-500/20 rounded-lg text-xs font-semibold hover:bg-green-500 hover:text-white transition-all">Restaurar</button>
                        <button onClick={() => handleForceDeleteUser(item.id)} className="px-3 py-1.5 bg-red-600/20 text-red-400 border border-red-500/30 rounded-lg text-xs font-semibold hover:bg-red-600 hover:text-white transition-all">Borrado Permanente</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => { setCurrentEntity(item); setModalType("ver"); }} className="px-2.5 py-1.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-lg text-xs font-semibold hover:bg-blue-500 hover:text-white transition-all">Ver</button>
                        <button onClick={() => { setCurrentEntity(item); setModalType("editar"); }} className="px-2.5 py-1.5 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 rounded-lg text-xs font-semibold hover:bg-yellow-500 hover:text-white transition-all">Editar</button>
                        <button onClick={() => handleDelete(item.id)} className="px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg text-xs font-semibold hover:bg-red-500 hover:text-white transition-all">Eliminar</button>
                      </>
                    )}
                  </td>
                </tr>
